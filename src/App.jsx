...
const MobileNav = ({ activeTab, onTabChange, onQuickAdd }) => {
    ...
    return (
        <div className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-slate-200 flex justify-between px-6 py-2 pb-6 z-50 shadow-up">
            {items.map(item => (
                <button
                    key={item.id}
                    onClick={() => onTabChange(item.id)}
...
function App() {
    ...
    // Data State (Loaded via Service)
    const [budgetData, setBudgetData] = useState(() => {
        const defaultBudget = {};
        MONTHS.forEach((_, i) => defaultBudget[i] = JSON.parse(JSON.stringify(INITIAL_CATEGORIES)));
        return storageService.getBudget(defaultBudget);
    });

    const [assets, setAssets] = useState(() => storageService.getAssets([]));
    const [installments, setInstallments] = useState(() => storageService.getInstallments([]));
    const [transactions, setTransactions] = useState(() => storageService.getTransactions([]));

    // Persistence (Saved via Service)
    useEffect(() => storageService.saveBudget(budgetData), [budgetData]);
